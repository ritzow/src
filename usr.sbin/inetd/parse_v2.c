...
typedef enum values_state
	{VALS_PARSING, VALS_END_KEY, VALS_END_DEF, VALS_ERROR} values_state;
...
typedef enum invoke_result 
	{INVOKE_SUCCESS, INVOKE_FINISH, INVOKE_ERROR} invoke_result;

/* The result of a parse of key handler values */
typedef enum hresult
	{KEY_HANDLER_FAILURE, KEY_HANDLER_SUCCESS} hresult;
...
parse_syntax_v2(struct servtab *sep, char **cpp)
{
	...
	for (;;) {
		...
		case INVOKE_ERROR:
			if (debug) {
				fprintf(stderr, "Syntax error, exiting config\n");
			}
			return V2_ERROR;
		}
	}
...
socket_type_handler(struct servtab *sep, vlist values)
{
	...
	if (sep->se_socktype == -1) {
		ERR("Invalid socket type '%s'. Valid: " VALID_SOCKET_TYPES, type);
		return KEY_HANDLER_FAILURE;
	}
...
filter_handler(struct servtab *sep, vlist values)
{
	...
	if (af_arg != NULL) {
		strlcpy(sep->se_accf.af_arg, af_arg, sizeof(sep->se_accf.af_arg));
		if (next_value(values) != NULL) {
			TMA("filter");
			return KEY_HANDLER_FAILURE;
		}
